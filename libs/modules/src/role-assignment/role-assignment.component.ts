...
export class RoleAssignmentComponent implements OnInit {
  ...
  tableColumns: string[] = ['user', 'warehouse'];
  ...
  // NEW REWORK FUNCTIONS

  async ngOnInit() {
    await this.intermediaryService.presentLoading('Cargando...');
    await this.usersService.getList().then(response => {
      ...
      if (this.tableRoles.length > 0) {
        this.tableRoles.forEach((rol) => {
          this.tableColumns.push(rol.id.toString());
        });
      }
    });
    await this.intermediaryService.dismissLoading();
  ...
  thereAreChanges(): boolean {
    return this.listToUpdate.length > 0;
  }

  isFiltering(column: string): string {
    switch(column){
      case 'user':
        if(this.filters.usersSelected.length > 0){
          return 'playlist_add_check';
        }else{
          return 'reorder';
        }
      case 'warehouse':
        if(this.filters.warehousesSelected.length > 0){
          return 'playlist_add_check';
        }else{
          return 'reorder';
        }
