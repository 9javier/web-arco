    ...
    this.showLoading('Iniciando sesión...').then(() => {
      this.loginService.post_login(user, AppInfo.Name.Al).subscribe(
        (data: HttpResponse<ResponseLogin>) => {
          ...
          this.authenticationService.login(data.body.data.access_token, data.body.data.user.id,data.body.data.accessPermitionsDictionary,data.body.data.refresh_token);
